...
이후 모든 단계는 `.status/제안 파트/{기회명}/.status`의 `outputs.rfp-context` URL을 우선 참조한다.
...
- 완료 조건: `.status/제안 파트/{기회명}/.status`의 `outputs.opportunity-analysis` URL 존재

### Step B: 시장/경쟁사 분석

- 입력: Step A 제안 분석 보고서 (`.status outputs.opportunity-analysis` Drive URL)
- 처리: 전략팀 L3에 위임 → RFP 분석해서 아데오에서 제안 진행 가능여부 판단 및 제안 전략 수립
- 완료 조건: `.status/제안 파트/{기회명}/.status`의 `outputs.proposal-strategy` URL 존재
...
- 완료 조건: `.status/제안 파트/{기회명}/.status`의 `outputs.proposal` Figma URL 존재
...
- 완료 조건: `.status/제안 파트/{기회명}/.status`의 `outputs.policy-fund` Drive URL 존재
