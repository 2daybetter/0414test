...
- **처리 내용**:
  ...
  6. 서비스 컨셉 방향 정리
- **출력 방법**: `strategy-analyzer` 스킬 → `mcp__claude_ai_Google_Drive__create_file` 업로드 → URL을 Google Drive 아데오 프로젝트/{프로젝트명}/.status/.status 파일의 `outputs.requirements`에 기록
...
- **출력 방법**: `ia-generator` 스킬 → `mcp__claude_ai_Google_Drive__create_file` 업로드 → URL을 Google Drive 아데오 프로젝트/{프로젝트명}/.status/.status 파일의 `outputs.ia`에 기록
