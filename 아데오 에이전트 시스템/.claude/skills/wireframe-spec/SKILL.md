...
IA 설계서(`ia-{프로젝트명}.md`)를 읽어 다음 데이터를 추출한다.

**추출 대상 (FO · BO 각각)**:
- 1depth 메뉴 목록 → Section 생성에 사용
- 2depth 메뉴 목록 (Type = Page 한정) → Frame 생성에 사용
- 각 Page의 화면ID / 화면명 / URL / 기능 요약

**파싱 결과 예시**:
```
[FO]
1depth: 회사소개 → Section "회사소개"
  2depth (Page): 인사말 (FO-1-01) → Frame "인사말"
  2depth (Page): 연혁 (FO-1-02) → Frame "연혁"
  2depth (Page): 오시는길 (FO-1-03) → Frame "오시는길"

1depth: 사업영역 → Section "사업영역"
  2depth (Page): 사업영역 소개 (FO-2-01) → Frame "사업영역 소개"
  ...
```
...
| Frame 명칭 | `{화면ID} {화면명}` (예: `FO-1-01 인사말`) |
...
|------|---|
| 크기 | 1440 × 900 px |
| Frame 명칭 | `{화면ID} {화면명}` (예: `BO-1-01 대시보드`) |
